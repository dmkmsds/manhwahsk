...
if "google_cloud" in st.secrets:
    ...
    with open(creds_path, "w") as f:
        json.dump(st.secrets["google_cloud"], f)
...
def google_translate(text_en):
    ...
    Returns a tuple: (translated text, alignment placeholder).
    """
...
def draw_wrapped_lines(draw, lines, font, start_x, start_y, max_width):
    """
    Draws wrapped lines (list of token lists) starting at (start_x, start_y).
    Each line is horizontally centered within max_width.
...
def detect_text_boxes(image_path):
    """Detect text bounding boxes using Google Vision."""
...
def awesome_align(english_text, chinese_text):
    """
    Compute alignment between English and Chinese using a dot-product/softmax method.
    Returns a string representation of a set of tuples: {(src_index, tgt_index, avg_probability), ...}
    """
    align_layer = 8
    th = 1e-3
    ...
    ids_src = tokenizer.prepare_for_model(list(itertools.chain(*wid_src)),
                                          return_tensors='pt',
                                          model_max_length=tokenizer.model_max_length,
                                          truncation=True)['input_ids']
    ids_tgt = tokenizer.prepare_for_model(list(itertools.chain(*wid_tgt)),
                                          return_tensors='pt',
                                          model_max_length=tokenizer.model_max_length,
                                          truncation=True)['input_ids']
    sub2word_map_src = []
...
def translate_to_segments(english_text):
    """
    1. Checks for Korean content:
         - If the entire text is Korean, returns None (no overlay drawn).
         - Otherwise, filters out Korean tokens.
    2. Translates the filtered English text to Chinese.
    3. Uses Awesome-Align to obtain the alignment mapping.
    4. Tokenizes the filtered English sentence and segmented Chinese sentence.
    5. Assigns colors using a palette of 10 colors.
    6. Returns a tuple with segmentation lists (for English, Mandarin, and pinyin),
       the raw mapping string, and the Chinese translation.
    """
...
def overlay_merged_pinyin(image_path, items, font_path=FONT_PATH, margin=MARGIN):
    """
    For each annotation box, clear its background and overlay translation text.
    Displays pinyin on top, a separator, then English below.
    Adjusts font size to fit. Skips overlay if OCR text is all Korean.
    """
    EXTRA_PADDING = 10
...
def main():
    ...
    # Create a container at the top for the download button.
    ...
    if uploaded_files:
        ...
        # Package all processed CBZ files into a single ZIP.
